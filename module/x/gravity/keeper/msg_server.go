...
func (k msgServer) SetDelegateKeys(c context.Context, msg *types.MsgDelegateKeys) (*types.MsgDelegateKeysResponse, error) {
	...
	if _, err := k.Keeper.StakingKeeper.Validator(ctx, valAddr); err != nil {
...
func (k Keeper) getSignerValidator(ctx sdk.Context, signerString string) (sdk.ValAddress, error) {
	...
	if validator := k.GetOrchestratorValidatorAddress(ctx, signer); validator == nil {
		validatorI, err = k.StakingKeeper.Validator(ctx, sdk.ValAddress(signer))
		if err != nil {
			return nil, err
		}
	} else {
		validatorI, err = k.StakingKeeper.Validator(ctx, validator)
		if err != nil {
			return nil, err
		}
	}
	...
	valAddr, err := sdk.ValAddressFromBech32(validatorI.GetOperator())
	return valAddr, err
}
